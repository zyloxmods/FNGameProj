...
void UFortMcpContext::PurchaseRealMoneyCatalogEntry(const FString& OfferId, const FOnRealMoneyPurchaseComplete& Callback) {
}
...
EOfferPurchaseError UFortMcpContext::PurchaseOffer(const FString& OfferId, const int32 PriceIndex, const FOnPurchaseOfferComplete& Callback, int32 Quantity) {
    return EOfferPurchaseError::NoError;
}
...
FString UFortMcpContext::GetAppName() const {
    return TEXT("");
}
...
void UFortMcpContext::DownloadEULA(const UFortMcpContext::FOnDownloadEULAComplete OnCompleteDelegate) {
}
...
UFortMcpContext::UFortMcpContext() {
    this->MinTimeBetweenFriendCodeRequestsSeconds = 1800.00f;
}
